            ...
            .on('changed.jstree', function(e, data) {
                ...
                for (i = 0, j = data.selected.length; i < j; i++) {
                    ...
                    if (data.instance.get_node(data.selected[i]).parent) {
                        const header = `<button class="btn btn-outline-warning btn-sm" onclick="clickInf()" id="inf">Informações</button>
                                <button class="btn btn-outline-warning btn-sm" onclick="diretorios()" id="dir">&nbspDiretórios&nbsp</button>`
